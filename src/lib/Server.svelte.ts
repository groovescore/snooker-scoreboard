...
export class Server {
  ...
  constructor(server_url: string, server_key: string) {
  ...
  async get_fixtures() {
    const url: string = `${this.server_url}/api/v1/fixtures`;
    const headers: Headers = new Headers();

    headers.append('Accept', 'application/json');
    headers.append('Content-Type', 'text/plain');
    // FIXME: Authorization: Basic <key>
    headers.append('Authorization', `${this.server_key}`);
  ...
  async post_scores(game: Game) {
    const url: string = `${this.server_url}/api/v1/scores/${game.state.league_match_id}`
    const headers: Headers = new Headers();
